// skidded and modified meteor nuker
...
public class SignAura extends Module {
    ...
    public SignAura() {
        super(LUtils.CATEGORY, "sign aura", "fuck lide.");
    }
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        BlockIterator.register(Math.max((int) Math.ceil(range.get() + 1), maxh), Math.max((int) Math.ceil(range.get()), maxv), (blockPos, blockState) -> {
            ...
            if (blockEntity instanceof SignBlockEntity sign) {
                ...
                for (Text line : lines) for (String str : strings.get()) if(line.getString().contains(str)) {
    ...
    private void compileFilterRegexList() {
        ...
        for (int i = 0; i < strings.get().size(); i++) {
            try {
                filterRegexList.add(Pattern.compile(strings.get().get(i)));
            } catch (PatternSyntaxException e) {
                String removed = strings.get().remove(i);
